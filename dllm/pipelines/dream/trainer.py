...
def cart_weight(
    ...
    Args:
        masked_indices (torch.Tensor): (b, seq_len) bool tensor indicating masked positions.
    ...
    Returns:
        torch.Tensor: (b, seq_len) float tensor of weights.
    """
    b, seq_len = masked_indices.shape
    device = masked_indices.device

    idx = torch.arange(seq_len, device=device)
    dist_matrix = (idx[None, :] - idx[:, None]).abs() - 1
    dist_matrix = torch.clamp(dist_matrix, min=0)  # (seq_len, seq_len)
    geo_matrix = (
    ...
    valid_mask = (~masked_indices).float()  # (b, seq_len), 1 = unmasked
    weights = valid_mask @ geo_matrix.T  # (b, seq_len)
