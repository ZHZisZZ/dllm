...
class NemotronDiffusionSamplerOutput(BaseSamplerOutput):
    nfe: int | None = None
    hidden_embeddings: list[torch.Tensor] | None = None
...
class NemotronDiffusionSamplerConfig(BaseSamplerConfig):
    ...
    return_hidden_embeddings: bool = False
...
class NemotronDiffusionSampler(BaseSampler):
    ...
    def sample(
        ...
        **kwargs,
        ...
        for attr in ("model", "base_model"):
            ...
            if candidate is not None and candidate is not model and callable(getattr(candidate, "__call__", None)):
                model = candidate
                break

        device = torch.device(self.model.device)
        if isinstance(inputs, torch.Tensor):
            prompt_ids = inputs.to(device=device, dtype=torch.long)
            if prompt_ids.dim() == 1:
                prompt_ids = prompt_ids[None]
        else:
            if inputs and all(isinstance(token_id, int) for token_id in inputs):
                inputs = [inputs]
            tensors = [
            ...
            if not tensors:
                raise ValueError("NemotronDiffusionSampler requires at least one input prompt.")
            max_prompt_len = max(tensor.numel() for tensor in tensors)
            if all(tensor.numel() == max_prompt_len for tensor in tensors):
                prompt_ids = torch.stack([tensor.reshape(-1) for tensor in tensors], dim=0)
            else:
                pad_token_id = config.pad_token_id
                if pad_token_id is None:
                    pad_token_id = getattr(self.tokenizer, "pad_token_id", None)
                if pad_token_id is None:
                    raise ValueError("Variable-length prompts require pad_token_id in config or tokenizer.")
                ...
                for i, tensor in enumerate(tensors):
                    prompt_ids[i, : tensor.numel()] = tensor.reshape(-1)
        ...
        end_think_token_id = config.end_think_token_id
        if end_think_token_id is None and config.max_thinking_tokens is not None:
            encode = getattr(self.tokenizer, "encode", None)
            if callable(encode):
                ids = encode("</think>", add_special_tokens=False)
                if ids:
                    end_think_token_id = int(ids[-1])
        ...
        causal_context = bool(config.causal_context)
        hidden_embeddings = [] if config.return_hidden_embeddings else None
        nfe = 0
        ...
        output = model(prompt_ids, use_cache=True, use_causal_mask=causal_context)
        past_key_values = output.past_key_values
        if hidden_embeddings is not None:
            hidden_embeddings.append(_hidden_embedding(output))
        if causal_context:
        ...
        for num_block in range(num_blocks):
            ...
            if end_think_token_id is not None and config.max_thinking_tokens is not None:
                tokens_before = num_block * block_length
                tokens_after = tokens_before + block_length
                if tokens_after > config.max_thinking_tokens:
                    gen_so_far = x_accum[:, prompt_ids.size(1):block_start]
                    has_end_think = (
                        (gen_so_far == end_think_token_id).any(dim=1)
                        if gen_so_far.size(1) > 0
                        else torch.zeros(batch_size, dtype=torch.bool, device=prompt_ids.device)
                    )
                    if not has_end_think.all():
                        offset = max(0, config.max_thinking_tokens - tokens_before)
    ...
    def infill(
        ...
        **kwargs,
        ...
        raise NotImplementedError("Nemotron diffusion sampler only exposes generate/sample.")


def _hidden_embedding(output) -> torch.Tensor | None:
    if getattr(output, "hidden_states", None):
        return output.hidden_states[-1].detach().clone()
    if getattr(output, "last_hidden_state", None) is not None:
        return output.last_hidden_state.detach().clone()
    return None
